...
router.get("/", authMiddlewarePage, async (req, res) => {
  const types = await prisma.unifiedAgent.findMany(
    {
...
router.post("/", authMiddlewarePage, async (req, res) => {
  const { name } = req.body;
  const newType = await prisma.unifiedAgent.create({ data: { name } });
  res.json(newType);
});
...
router.delete("/:id", authMiddlewarePage, async (req, res) => {
  const { id } = req.params;
  try {
    const deleted = await prisma.unifiedAgent.delete({ where: { id: Number(id) } });
    res.json(deleted);
  } catch (error) {
...
router.put("/:id", authMiddlewarePage, async (req, res) => {
  ...
  try {
    const updatedType = await prisma.unifiedAgent.update({
      where: { id: Number(id) },
