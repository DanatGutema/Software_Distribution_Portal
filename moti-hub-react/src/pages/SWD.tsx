...
const saveRemark = async (id: number) => {
  try {
    ...
    await api.put(`/swd/${id}`, { name: type!.name, remark: remarkValue });
  ...
  const addType = async () => {
    ...
    try {
    await api.post("/swd", { name: newType });
    setNewType("");
    fetchTypes();
  } catch (error) {
  ...
  const editType = async() =>{
    ...
    try {
      ...
      await api.put(`/swd/${selectedTypeId}`, { name: newName.trim(), remark: typeToEdit.remark ?? "" });
      fetchTypes();
    } catch (err) {
  ...
  const deleteType = async () => {
    ...
    try {
      ...
      await api.delete(`/swd/${selectedTypeId}`);
  ...
  return (
    <div className="p-5">
      <h2 className="text-lg font-bold mb-3">Manage SWD</h2>
      <table className="w-full border">
        <thead>
          <tr className="bg-gray-100">
            <th className="border p-2 w-50">ID Number</th>
            <th className="border p-2 w-100">SWD</th>
