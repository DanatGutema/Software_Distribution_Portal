...
const saveRemark = async (id: number) => {
  try {
    ...
    await api.put(`/xfs/${id}`, { name: type!.name, remark: remarkValue });
  ...
  const addType = async () => {
    ...
    try {
    await api.post("/xfs", { name: newType });
    setNewType("");
    fetchTypes();
  } catch (error) {
  ...
  const editType = async() =>{
    ...
    try {
      ...
      await api.put(`/xfs/${selectedTypeId}`, { name: newName.trim(), remark: typeToEdit.remark ?? "" });
      fetchTypes();
    } catch (err) {
  ...
  const deleteType = async () => {
    ...
    try {
      ...
      await api.delete(`/xfs/${selectedTypeId}`);
  ...
  return (
    <div className="p-5">
      <h2 className="text-lg font-bold mb-3">Manage XFS</h2>
      <table className="w-full border">
        <thead>
          <tr className="bg-gray-100">
            <th className="border p-2 w-50">ID Number</th>
            <th className="border p-2 w-100">XFS</th>
